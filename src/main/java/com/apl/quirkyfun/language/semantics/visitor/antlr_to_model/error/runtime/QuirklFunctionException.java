...
public class QuirklFunctionException extends QuirklTypeException {
    ...
    private static QuirklFunctionException newInstance(String message, QuirklCoordinate coord) {
        return new QuirklFunctionException(message + getCoord(coord));
    }

    public static QuirklFunctionException invalidReturnType(QuirklCoordinate coord, Function func, QuirklType.TYPE resultType) {
        String msg = String.format("Cannot return %s as %s for anonymous function", resultType, func.getType());
        return newInstance(msg, coord);
    }

    public static QuirklFunctionException invalidNumberOfArguments(QuirklCoordinate coord, Function func, int expected, int actual) {
        String msg = String.format("Function %s expects %d arguments, but %d were provided", func.getId(), expected, actual);
        return newInstance(msg, coord);
    }

    public static QuirklFunctionException incompatibleReturnTypes(QuirklCoordinate coord, Function func1, Function func2) {
    ...
    public static QuirklFunctionException tooManyArguments(QuirklCoordinate coord, Function func, int expected, int actual) {
        String msg = String.format("Function %s expects %d arguments, but %d were provided", func.getId(), expected, actual);
        return newInstance(msg, coord);
    }

    public static QuirklFunctionException incompatibleParameter(QuirklCoordinate coord, Function func1, Function func2) {
    ...
    public static QuirklFunctionException returnButVoid(QuirklCoordinate coord, Function func) {
        String msg = String.format("Function %s is void and cannot return a value", func.getId());
        return newInstance(msg, coord);
    }

    public static QuirklFunctionException noReturn(QuirklCoordinate coord, Function func) {
        String msg = String.format("Function %s must return a value", func.getId());
        return newInstance(msg, coord);
    }

    public static QuirklFunctionException notAFunction(QuirklCoordinate coord, Variable var) {
